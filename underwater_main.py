...
from underwater_functions import check_input, adjust_gamma, RGB2Lab, ColorCorrec, Lab2RGB, normalize, constrast_correction_lab, normalizeBis, normalizeTer
...
for file in file_list:
    ...
    if args.colour_correction == "LAB":
        ...
        if args.lab_constrast_correction == "CLAHE":
            ...
            clahe = cv2.createCLAHE(clipLimit=1.0,tileGridSize=(8,8))
            lab_planes[0] = clahe.apply(lab_planes[0])
            imCIELabCorrec = cv2.merge(lab_planes)
            imRGBCorrec = cv2.cvtColor(lab, cv2.COLOR_LAB2RGB)
